...
def write_global_overview(repo: Path, lanes: list[Lane]) -> None:
    ...
    lines: list[str] = []
    lines.append("# OCCT research overview")
    lines.append("")
    ...
    for lane in lanes:
        ...
        lines.append(
            f"| `{lane.slug}` ([hub]({hub_path})) | {focus} | {map_cell} | {dossier_cell} | {repro_cell} | {backlog_cell} |"
        )
    ...
    lines.append("")
    lines.append("1) Start with the hub (`notes/maps/hub-<lane>.md`).")
...
def write_lane_hubs(repo: Path, lanes: list[Lane]) -> None:
    ...
    for lane in lanes:
        ...
        lines: list[str] = []
        lines.append(f"# Lane hub: {lane.slug}")
        lines.append("")
        lines.append("_Generated by `tools/gen_overview_pages.py`. Edit only inside `MANUAL:*` blocks._")
        lines.append("")
        ...
        if lane.anchor_symbols:
            for path, sym in lane.anchor_symbols:
                if sym:
        ...
        if tasks:
            ...
            for t in tasks:
                ...
                title = t.get("title", "")
                path = t.get("path", "")
                lines.append(f"- `{tid}` ({status}) — {title} (`{path}`)")
            lines.append("")
